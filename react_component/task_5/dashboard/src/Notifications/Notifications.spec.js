...
test('Should Check when simulating a click on a notification item display string "Notification {id} has been marked as read"', () => {
  let consoleLogSpy;
  ...
  render(<Notifications {...props} />)
  const notifications1 = screen.getByText('New course available');
  fireEvent.click(notifications1);
  expect(consoleLogSpy).toHaveBeenCalledWith('Notification 1 has been marked as read');

  const notifications2 = screen.getByText('New resume available');
  fireEvent.click(notifications2);
  expect(consoleLogSpy).toHaveBeenCalledWith('Notification 2 has been marked as read');
